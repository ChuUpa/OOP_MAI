...
TEST(RectangleTest, ConstructorCopy) {
    ...
    Rectangle<double> p1(points);
    Rectangle<double> p2 = p1;
    EXPECT_EQ(p1, p2);
}
...
TEST(RectangleTest, EqualityOperator) {
    ...
    Rectangle<double> p1(points1);
    Rectangle<double> p2(points1);
    EXPECT_TRUE(p1 == p2);
}
...
TEST(FigureArrayTest, AddAndTotalArea) {
    ...
    auto p = std::make_shared<Rectangle<double>>(points);
    array.add(p);
